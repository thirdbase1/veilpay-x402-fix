...
import { ledger, type Intent, type Ledger } from '../../contract/src/managed/veilpay/contract/index.js';
...
const loadSeed = (): string => {
  const envSeed = process.env.VEILPAY_SEED;
  if (envSeed) return envSeed.trim();
  if (fs.existsSync(seedFile())) return fs.readFileSync(seedFile(), 'utf8').trim();
...
const intentJson = (id: bigint, intent: Intent): string =>
  JSON.stringify(
    {
      id: id.toString(),
...
export const run = async (config: Config, argv: string[], logger: Logger): Promise<void> => {
  ...
  try {
    ...
    const zkConfigProvider = new NodeZkConfigProvider<'createIntent' | 'pay' | 'refund' | 'cancel'>(
      config.zkConfigPath,
    );
    ...
    switch (command) {
      case 'create': {
        ...
        console.log(
          JSON.stringify(
            { id: id.toString(), amount: amount.toString(), paymentSecret: toHex(paymentSecret) },
            null,
            2,
          ),
        );
