...
const deployedAddress = (): string | null => {
  ...
  if (fs.existsSync(ADDRESS_FILE)) return fs.readFileSync(ADDRESS_FILE, 'utf8').trim();
  return null;
};
...
const run = async (argv: string[]): Promise<void> => {
  ...
  const logger = await createLogger(
    path.resolve(STATE_DIR, '..', 'logs', 'preprod', `gateway-tx-${new Date().toISOString().slice(0, 10)}.log`),
  );
      ...
      case 'status': {
        ...
        if (rest[0]) {
          ...
          for (let i = 1n; i <= l.sequence; i++) {
            if (l.intents.member(i)) intents.push(JSON.parse(intentJson(i, l.intents.lookup(i))));
          }
          console.log(
            JSON.stringify(
