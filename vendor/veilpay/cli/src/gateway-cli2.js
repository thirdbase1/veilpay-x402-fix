...
const hexToBytes = (s) => new Uint8Array(Buffer.from(s.replace(/^0x/, ''), 'hex'));
const intentJson = (id, intent, hasReceipt) => JSON.stringify({
    id: id.toString(),
...
const run = async (argv) => {
    ...
    const logger = await createLogger(path.resolve(STATE_DIR, '..', 'logs', 'preprod', `gateway2-tx-${new Date().toISOString().slice(0, 10)}.log`));
    ...
    try {
        ...
        switch (command) {
            case 'create': {
                ...
                if (merchantCoinPk.length !== 32)
                    throw new Error('unexpected coin public key length');
